...
def main(p_dict):
    ...
    for chrom_str in list(cord_data_g.keys()):
        if not os.path.isfile(local_ld_dict_file+'-'+chrom_str):
            ...
            print('new 0',time.time())

            futures=[]
            snps=[1]*numCores
            with ProcessPoolExecutor(numCores) as executor:
                for core in range(numCores):
                    rowRange=np.arange(core*np.ceil(snp_means.shape[0]/numCores),min(snp_means.shape[0],(core+1)*int(np.ceil(
                        snp_means.shape[0]/numCores)))).astype(int)
                    futures+=[executor.submit(CAnnoPred.processRawSnps,raw_snps[rowRange],snp_means[rowRange], snp_stds[rowRange],core)]

                for f in as_completed(futures):
                    ans=f.result()
                    snps[ans['core']]=ans['snps']
                    
            print('new 1',time.time())
            snps=np.concatenate(snps,axis=0)    
            M=snps.shape[0]
            ld_radius=p_dict['ld_radius']
            window_size=2*p_dict['ld_radius']
            ret_dict={'ld_scores':[1]*M,'ld_mat':[1]*M,'ref_ld_matrices':[1]*int(1+np.floor((M-1.0)/max(1,2*p_dict['ld_radius'])))}
            print('new 2',time.time())

            futures=[]
            with ProcessPoolExecutor(numCores) as executor:
                for core in range(numCores):
                    start=core*int(np.ceil(snp_means.shape[0]/numCores))
                    stop=min(snp_means.shape[0],(core+1)*int(np.ceil(snp_means.shape[0]/numCores)))
                    
                    futures+=[executor.submit(CAnnoPred.ldMatrices,snps,start,stop,ld_radius)]
                    
                for f in as_completed(futures):
                    ans=f.result()
                    ret_dict['ld_scores'][ans['begin']:ans['end']]=ans['ld_scores']
                    ret_dict['ld_mat'][ans['begin']:ans['end']]=ans['ld_matrices']
                    
            print('new 3',time.time())
            futures=[]
            with ProcessPoolExecutor(numCores) as executor:
                for core in range(numCores):
                    start=core*int(np.ceil(snp_means.shape[0]/numCores))
                    ...
                    futures+=[executor.submit(CAnnoPred.refLdMatrices,snps,start,stop,window_size)]
                    
                for f in as_completed(futures):
                    ans=f.result()
                    ret_dict['ref_ld_matrices'][ans['begin']:ans['end']]=ans['ref_ld_matrices']
                
            print('mid',time.time())
            ret_dict_old = get_LDpred_ld_tables_old(raw_snps,snp_means, snp_stds,p_dict['ld_radius'], 2*p_dict['ld_radius'])
            print('old',time.time())
