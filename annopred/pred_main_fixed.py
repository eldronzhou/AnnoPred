...
def get_LDpred_ld_tables(snps,ld_radius=100,ld_window_size=0):
    t0=time.time()
    old=get_LDpred_ld_tables_old(snps, ld_radius, ld_window_size)
    t1=time.time()
    ret_dict=CAnnoPred.get_LDpred_ld_tables(snps, ld_radius, ld_window_size)
    t2=time.time()
...
#@profile
def get_LDpred_ld_tables_old(snps, ld_radius=100, ld_window_size=0):
    """
