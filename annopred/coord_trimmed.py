...
def parse_sum_stats_basic(filename=None,
    ...
    """
    bimDF=pd.read_csv(bimfile,sep='\t',index_col=None,header=None)
    sortValSids=bimDF.iloc[:,1].sort_values().values.flatten()
    ...
    sqrtN=np.sqrt(n)
    for chrom in df.iloc[:,0].drop_duplicates().values.flatten():
        t_df=df[df.iloc[:,0]==chrom].copy()
        sids=t_df.iloc[:,1].values.flatten().astype(str)
        sidOrd=np.argsort(sids)
        sortSids=sids[sidOrd]
        ss=np.minimum(np.searchsorted(sortValSids,sortSids),len(sortValSids)-1)
        loc=np.sort(sidOrd[sortSids==sortValSids[ss]])
        t_df=t_df.iloc[loc]
        log_odds=np.log(t_df.iloc[:,5]).values.flatten()
        pval=t_df.iloc[:,6].values.flatten()

        chrom_dict[chrom]={
            'ps':pval.tolist(),
            'log_odds':log_odds.tolist(),
            'infos':[],
            'betas':(-np.sign(log_odds)*stats.norm.ppf(pval/2.0)/sqrtN).tolist(),
            'nts':t_df.iloc[:,2:4].values.tolist(),
            'sids':t_df.iloc[:,1].values.flatten().tolist(),
            'positions':t_df.iloc[:,4].astype(int).values.flatten().tolist()
        }
    ...
    for chrom in chrom_dict.keys():
        ...
        sl = sorted(zip(chrom_dict[chrom]['positions'], chrom_dict[chrom]['sids'], chrom_dict[chrom]['nts'],
        ...
        g.create_dataset('nts', data=[[x.encode('utf8'),y.encode('utf8')] for x,y in nts])
        g.create_dataset('sids', data=[x.encode('utf8') for x in sids])
...
#   recode_dict = {1:'A', 2:'T', 3:'C', 4:'G'} #1K genomes recoding..
    ...
    fids = [s.fid.encode('utf8') for s in samples]
    iids = [s.iid.encode('utf8') for s in samples]
...
#     chr_dict_ref = _get_chrom_dict_bim_(reference_genotype_file+'.bim', chromosomes)
    ...
    for chrom in chromosomes:
        ...
        ss_sids = [x.decode('utf8') for x in ssg['sids'][...]]
...
#         rg_nts_ok = sp.array(rg_nts_l)
        ...
        ss_nts = np.array([[x.decode('utf8'),y.decode('utf8')] for x,y in ssg['nts'][...]])
        betas = ssg['betas'][...]
        log_odds = ssg['log_odds'][...]
...
#         ok_indices['ss'] = list(sp.array(ok_indices['ss'])[order])
        ...
        ps = ssg['ps'][...][ok_indices['ss']]
        nts = sp.array(ok_nts)#[order]
        sids = np.array([x.decode('utf8') for x in ssg['sids'][...][ok_indices['ss']]])
...
#         assert sp.all(g_sids==rg_sids) and sp.all(rg_sids==ss_sids), 'WTF?'
        ...
        ofg.create_dataset('nts', data=[[x.encode('utf8'),y.encode('utf8')] for x,y in nts])
        ofg.create_dataset('ps', data=ps)
        ofg.create_dataset('positions', data=positions)
        ofg.create_dataset('sids', data=[x.encode('utf8') for x in sids])
        if genetic_map_dir is not None:
            ofg.create_dataset('genetic_map', data=genetic_map)
        ofg.create_dataset('betas', data=betas)
        ofg.create_dataset('log_odds', data=log_odds)
